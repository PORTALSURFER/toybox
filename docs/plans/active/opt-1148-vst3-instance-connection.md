...
- Use the host-provided VST3 `IConnectionPoint` pairing rather than plugin creation order.
...
- Plugin processor and controller classes add `IConnectionPoint` and Toybox's private shared-state
  bridge to their COM interface lists.
...
same preflight with the repository's local SDK path passes.
