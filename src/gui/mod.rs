...
pub mod declarative {
    pub use patchbay_gui::{
        ...
        CurveEditorSpec, CurveHighlightMode, CurveInteractionOptions, CurveModel, CurvePoint,
        CurveSegment, DeclarativeError, DropdownSpec, EdgeInsets, EndpointMode, FlexSpec, GridKind,
        ...
        SlotParams, SlotSpec, SpacingTokens, StackSpec, SurfaceCommand, SwitchCase,
        SwitchLayoutSpec, SwitchWidthRange, TextBoxSpec, ThemeTokens, ToggleSpec, TrackSize,
        TypographyTokens, UiAction, UiInvalidationScope, UiSpec, WeightedSlot, WrapSpec, button,
        column, column_slots, curve_editor, dropdown, fill_slot, fraction_slot, grid, indicator,
        knob, measure_checked, panel, region, render_checked, render_checked_with_engine,
        root_frame_sized, row, row_slots, scroll_view, slider, slot, spacer, stack, surface,
        switch_layout, textbox, toggle, weighted_slot, weighted_slot_lengths, when_width_between,
        when_width_ge, when_width_lt, wrap,
    };
}
