...
impl<G: Vst3HostedGui> IPlugViewTrait for HostedVst3View<G> {
    ...
    unsafe fn attached(&self, parent: *mut std::ffi::c_void, r#type: FIDString) -> tresult {
        self.guarded_callback("VST3 attached", || {
            if parent.is_null() {
                return kInvalidArgument;
            }

            let Some(parent_handle) = (unsafe { parent_to_raw_window_handle(parent, r#type) })
            else {
                return kResultFalse;
            };
            ...
            let (requested_host_width, requested_host_height) = if let Some((width, height)) =
                gui.last_size()
            {
                (width, height)
            } else {
            ...
            let (constrained_host_width, constrained_host_height) =
                gui.host_size_from_logical(constrained_width as u32, constrained_height as u32);
    ...
    unsafe fn onWheel(&self, _distance: f32) -> tresult {
        self.guarded_callback("VST3 onWheel", || kResultFalse)
    }
    ...
    unsafe fn getSize(&self, size: *mut ViewRect) -> tresult {
        self.guarded_callback("VST3 getSize", || {
            if size.is_null() {
                return kInvalidArgument;
            }
    ...
    unsafe fn onSize(&self, new_size: *mut ViewRect) -> tresult {
        self.guarded_callback("VST3 onSize", || {
            if new_size.is_null() {
                return kInvalidArgument;
            }
            ...
            let (requested_width, requested_height) = gui
                .logical_size_from_host(requested_host_width as u32, requested_host_height as u32);
            let current = self.current_logical_size_for_gui(&gui);
            let (constrained_width, constrained_height) = self.constrain_uniform_size_from_current(
                logical_dimension(requested_width),
                logical_dimension(requested_height),
    ...
    unsafe fn setFrame(&self, _frame: *mut IPlugFrame) -> tresult {
        self.guarded_callback("VST3 setFrame", || kResultOk)
    }

    unsafe fn canResize(&self) -> tresult {
        self.guarded_callback("VST3 canResize", || kResultTrue)
    }

    unsafe fn checkSizeConstraint(&self, rect: *mut ViewRect) -> tresult {
        self.guarded_callback("VST3 checkSizeConstraint", || {
            if rect.is_null() {
                return kInvalidArgument;
            }
