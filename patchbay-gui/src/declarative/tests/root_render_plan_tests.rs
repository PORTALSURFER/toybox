...
fn plan_root_render_uniform_fit_centers_letterboxed_content() {
    let spec = UiSpec::new(
        RootFrameSpec::new(
            "root",
            Node::Region(RegionSpec::new("plot").layout(LayoutBox::fixed(100, 50).max(100, 50))),
        )
        .padding(0)
...
fn plan_root_render_uniform_fit_keeps_shared_scale_for_non_integer_ratio() {
    let spec = UiSpec::new(
        RootFrameSpec::new(
            "root",
            Node::Region(RegionSpec::new("plot").layout(LayoutBox::fixed(101, 50).max(101, 50))),
        )
        .padding(0)
...
fn uniform_fit_layout_size_is_clamped_to_design_viewport() {
    let spec = UiSpec::new(
        RootFrameSpec::new(
            "root",
            Node::Region(RegionSpec::new("plot").layout(LayoutBox::fixed(10, 10).max(10, 10))),
        )
        .padding(0)
...
fn plan_root_render_tiny_surface_clamps_surface_rect_within_window_bounds() {
    let spec = UiSpec::new(
        RootFrameSpec::new(
            "root",
            Node::Region(RegionSpec::new("plot").layout(LayoutBox::fixed(420, 258).max(420, 258))),
        )
        .padding(0)
