...
#[test]
fn row_invalid_width_bounds_emit_constraint_normalized_diagnostic() {
    let size = Size {
        width: 160,
        height: 60,
    };
    let content = row(vec![label("wide").widget_layout(LayoutBox::auto().min(80, 8).max(20, 40))]);
    let spec = UiSpec::new(
        RootFrameSpec::new("root", content)
    ...
    assert!(result.layout_diagnostics.iter().any(|diagnostic| {
        diagnostic.container == LayoutContainerKind::Flex
            && diagnostic.code == LayoutDiagnosticCode::ConstraintNormalized
            && diagnostic.message == "layout width min constraint exceeded max; normalized to max"
    }));
    assert_eq!(result.overflow.total, 0);
}

#[test]
fn column_invalid_height_bounds_emit_constraint_normalized_diagnostic() {
    let size = Size {
        width: 120,
        height: 120,
    };
    let content = column(vec![
        label("tall").widget_layout(LayoutBox::auto().min(8, 80).max(40, 20)),
    ]);
    let spec = UiSpec::new(
        RootFrameSpec::new("root", content)
            .layout(LayoutBox::fixed(size.width, size.height).max(size.width, size.height))
    ...
    assert!(result.layout_diagnostics.iter().any(|diagnostic| {
        diagnostic.container == LayoutContainerKind::Flex
            && diagnostic.code == LayoutDiagnosticCode::ConstraintNormalized
            && diagnostic.message == "layout height min constraint exceeded max; normalized to max"
    }));
    assert_eq!(result.overflow.total, 0);
}
