    ...
    fn on_event(&mut self, _window: &mut Window, event: Event) -> EventStatus {
        ...
        match &event {
            baseview::Event::Mouse(event) => match event {
                ...
                baseview::MouseEvent::DragEntered { data, .. }
                | baseview::MouseEvent::DragMoved { data, .. } => {
                    self.egui_input.hovered_files = drop_data_to_hovered_files(data);
                    return_status = EventStatus::AcceptDrop(baseview::DropEffect::Copy);
                }
                baseview::MouseEvent::DragLeft => {
                    self.egui_input.hovered_files.clear();
                }
                baseview::MouseEvent::DragDropped { data, .. } => {
                    self.egui_input.hovered_files.clear();
                    self.egui_input.dropped_files = drop_data_to_dropped_files(data);
                    return_status = EventStatus::AcceptDrop(baseview::DropEffect::Copy);
                }
                _ => {}
            },
...
fn drop_data_to_hovered_files(data: &baseview::DropData) -> Vec<egui::HoveredFile> {
    match data {
...
fn drop_data_to_dropped_files(data: &baseview::DropData) -> Vec<egui::DroppedFile> {
    match data {
        baseview::DropData::Files(paths) => paths
            .iter()
